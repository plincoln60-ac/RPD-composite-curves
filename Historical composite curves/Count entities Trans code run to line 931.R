...
RPDent1 <-  dbGetQuery(mydb, "select e.*, min(c.original_est_age), min(am.mean) from entity e 
...
where e.TYPE in('count', 'raw count') and c.original_est_age between -60 and 200
...
e <- unique(RPDent$ID_ENTITY)
paste0(e, collapse = ",")


RPDsite <- dbGetQuery(mydb, "SELECT s.* FROM site s
                      left join entity e on s.ID_SITE = e.ID_SITE 
                         WHERE ID_ENTITY in(1645,1646,1705,1723,1740,1741,1742,1784,1788,1789,1790,1792,1793,1794,1795,1796,1797,1798,1802,2031,2033,2042,2044,2045,2047,2053,2073,2282,2416,2434,2435,2436,2440,2441,2442,2444,2447,2448,2449,2450,2456,2457,2462,2464)")
...
write.csv(RPDdat, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Scount_entitiesat.csv", row.names = F)
write.csv(RPDsite, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/count_entitiessite.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/count_entitiesent.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/count_entitieschron.csv", row.names = F)
...
ent <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/count_entitiesent.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Scount_entitiesat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/count_entitieschron.csv' , na.strings = 'NULL')
...
dir.create("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/count_entities")
csvpath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/count_entities/"


#preserve only concentrations which you just got

data_query <- dat[,c("entity_name.x"   ,"ID_ENTITY", 'ID_SAMPLE',     "avg_depth"   ,      "original_est_age.y"   ,   "charcoal_measurement" ,  "TYPE.x" )] 
head(data_query)
...
for (j in maxsites[]) { ##trying looping using entity names: changed from j in 1:maxsites
  ...
  if (nsamp > 1) { ##I changed this from 0 to 1, because we have entities that are only one row in v4.
    ...
    else if (xst_level[1] == "COUN")     # calculate influx, adopt conc values as they are
    {
      conc <- quant
      conc_source <- "data"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/count_entities/",entid,"_RPD_", ".csv", sep="")
...
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/count_entities/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
transcsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/ZTrans_count_entities/",sep="") # 
transcsvpath

# if output folder does not exist, create it
queryname <- 'count_entities'
...
write.csv(ent_vec, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/count_entities/final_entities.csv")
