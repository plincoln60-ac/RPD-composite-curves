...
curvefile <- paste(curvecsvpath,"EURO_",basename,"_",binname,"_",as.character(hw),"_",as.character(nreps),'_',as.character(length(Zbin_dat)-1),'n', ".csv", sep="") #adjust to particular data you use now. .
print(curvefile)
...
if (plotout == "pdf") { ## when you define PDFfile over here, need to make sure you name it with the nsites and what baseperiod overlap level you insisted on.
  pdffile <- paste(curvecsvpath,"EURO_",basename,"_",binname,"_",as.character(hw),"_",as.character(nreps),'_',as.character(length(Zbin_dat)-1),'n','17_09_2021',".pdf", sep="")
  print(pdffile)
}
...
plot(x, y, xlab="Age (BP 1950)", ylab=fitname, xlim=c(-60, 200), xaxp  = c(0, 200, 10), ylim=c(-0.5,0.5), type="n") ##make sure xlim is right.
title(main = 'EURO_scores, bp = -60-200BP: min 50% overlap, bins = 27, hw = 10, nboot = 200', cex.main = 0.6)
...
colnames(loc01_fit) <- c("age", fitname)
lines(loc01_fit[,1], loc01_fit[,2], lwd=2, col="yellow") ##keep all 45-55n America red; 55-65N america blue, 65-75 america orange; 45-55 eurasia purple, 55-65 eurasia green, 65-75 eurasia pink.
...
lines(targage.df$x, yfit95, lwd=1, col="yellow")
lines(targage.df$x, yfit05, lwd=1, col="yellow")##keep all 45-55n America red; 55-65N america blue, 65-75 america orange; 45-55 eurasia purple, 55-65 eurasia green, 65-75 eurasia pink.
