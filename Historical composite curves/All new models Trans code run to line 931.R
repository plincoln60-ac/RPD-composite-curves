...
left join age_model am on am.ID_SAMPLE = s.ID_SAMPLE
left join chronology c on c.ID_SAMPLE = s.ID_SAMPLE
where am.mean between -60 and 200 
and TYPE != 'other')
...
write.csv(RPDdat, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelsdat.csv", row.names = F)
write.csv(RPDsite, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelssite.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelsent.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelschron.csv", row.names = F)
...
ent <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelsent.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelsdat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/new_modelschron.csv' , na.strings = 'NULL')
...
dir.create("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/new_models")
csvpath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/new_models/"
...
for (j in maxsites[]) { ##trying looping using entity names: changed from j in 1:maxsites
  ...
  if (nsamp > 1) { ##I changed this from 0 to 1, because we have entities that are only one row in v4.
    ...
    # select case based on xst_level
    if (xst_level[1] == "influx")          # adopt influx values as they are, calculate concentration - should be no influxes as input, but leaving in anyway
    {  
      influx <- quant
      influx_source <- "data"
      if (influx != miss && unit_dep_time != miss && sed_rate != 0.0) {
    ...
    else if (xst_level[1] == "concentration")     # calculate influx, adopt conc values as they are
    {
      conc <- quant
      conc_source <- "data"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
    ...
    else if (xst_level[1] == "pollen concentration")     # calculate influx, adopt conc values as they are
    {
      conc <- quant
      conc_source <- "data"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
    ...
    else if (xst_level[1] == "raw count")     # for now we are assuming that this is like concentration, but meeting on 19/03 to discuss this. (I changed this from "SOIL") - yes its concentration like according to sandy.
    {
      conc <- quant
      conc_source <- "data"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
        influx <- quant * sed_rate
        influx_source <- "calculated from raw count "
      } else {
        influx <- quant
        influx_source <- "copied from quant "
      }  
     ...
     else if (xst_level[1] == "count")     # for now we are assuming that this is like concentration, 
    {
      conc <- quant
      conc_source <- "data"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
        influx <- quant * sed_rate
        influx_source <- "calculated from count "
      } else {
        influx <- quant
        influx_source <- "copied from quant "
      }  
    ...
    else if (xst_level[1] == "charcoal area")     # for now we are assuming that this is like concentration, but meeting on 19/03 to discuss this. (I changed this from "SOIL") - yes its concentration like according to sandy.
    {
      conc <- quant
      conc_source <- "data"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
        influx <- quant * sed_rate
        influx_source <- "calculated from charcoal area "
      } else {
        influx <- quant
        influx_source <- "copied from quant "
      }  
  ...
  if (unique(xst_level) != 'count' & all(is.na(influx))) {
    ls2 <- append(ls2, j)
  }
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/new_models/",entid,"_RPD_", ".csv", sep="")
...
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/new_models/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
transcsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/ZTrans_new_models/",sep="") # 
transcsvpath

# if output folder does not exist, create it
queryname <- 'new_models'
...
ls1 <- c()
#Transform loop ----
for (z in 1:nsites[49:297]) {  #Do not change this counter to the name. very dangerous becuase the matrices you use are indexed by this counter as well.
...
for (j in 49:nsites) {  #looping through each .csv file of each site.
  ent_n<- ent_vec[j]
  ent_nm <- sites[sites$ID_ENTITY == ent_n,"entity_name"]
  entnmsub <- substr(ent_nm, start = 1, stop = 3)
...
write.csv(ent_vec, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/new_models/final_entities.csv")

##########################################  END OF MAIN LOOPING BIT ###################################################################

evec <- as.character(ent_vec, sep = ",")
evec
evec<- paste(ent_vec, collapse=", ")
