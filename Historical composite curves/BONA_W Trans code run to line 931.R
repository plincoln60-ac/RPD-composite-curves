...
RPDent <-  dbGetQuery(mydb, "SELECT * FROM entity WHERE ID_ENTITY in(29, 47, 51, 53, 63, 64, 146, 211, 213, 215, 217, 218, 219, 220, 221, 222, 224, 297, 300, 447, 449, 452, 454, 1782, 1344, 1424, 1425, 1531, 1361, 1431, 1436, 1470, 1383, 1384, 1446, 1551, 1448, 1451, 1559, 1453, 1412, 1555, 1489, 1519, 1547, 1607, 1677, 1735, 1780, 1791, 1799, 1829, 1830, 1832, 1834, 1883, 1884, 1885, 1886, 1887, 1888, 1889, 1890, 1891, 1892, 1893, 1894, 1895, 1896, 2054, 2057, 2076, 2088, 2089, 2091, 2238, 2239, 2424);") #longer site dataset is because of those from WP that have no entities.
str(RPDent)
...
RPDsite <- dbGetQuery(mydb, "SELECT s.* FROM site s
                      ...
                      WHERE e.ID_ENTITY in(29, 47, 51, 53, 63, 64, 146, 211, 213, 215, 217, 218, 219, 220, 221, 222, 224, 297, 300, 447, 449, 452, 454, 1782, 1344, 1424, 1425, 1531, 1361, 1431, 1436, 1470, 1383, 1384, 1446, 1551, 1448, 1451, 1559, 1453, 1412, 1555, 1489, 1519, 1547, 1607, 1677, 1735, 1780, 1791, 1799, 1829, 1830, 1832, 1834, 1883, 1884, 1885, 1886, 1887, 1888, 1889, 1890, 1891, 1892, 1893, 1894, 1895, 1896, 2054, 2057, 2076, 2088, 2089, 2091, 2238, 2239, 2424 );") 
...
write.csv(RPDdat, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Wdat.csv", row.names = F)
write.csv(RPDsite, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Wsite.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Went.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Wchron.csv", row.names = F)
...
ent <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Went.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Wdat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Wchron.csv' , na.strings = 'NULL')
...
dir.create("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_W")
csvpath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_W/"
...
for (j in maxsites[73]) { ##trying looping using entity names: changed from j in 1:maxsites
   nsamp <- 0
  sitedata <- data_query[data_query$ID_ENTITY == j, ]
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_W/",entid,"_RPD_", ".csv", sep="")
...
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_W/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
transcsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/ZTrans_BONA_W/",sep="") # 
transcsvpath
...
write.csv(ent_vec, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_W/final_entities.csv")
