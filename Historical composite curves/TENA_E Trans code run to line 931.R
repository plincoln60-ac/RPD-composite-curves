...
RPDent <-  dbGetQuery(mydb, "SELECT e.* FROM entity e 
                      WHERE ID_ENTITY in(45, 133, 134, 167, 179, 180, 182, 184, 188, 191, 199, 201, 206, 207, 208, 248, 249, 265, 280, 1485, 325, 327, 344, 346, 354, 359, 362, 363, 364, 369, 373, 374, 376, 377, 378, 379, 460, 461, 462, 464, 1454, 706, 801, 802, 825, 826, 1352, 1335, 1338, 1339, 1366, 1346, 1367, 1402, 960, 984, 1418, 1360, 1377, 1438, 1498, 1500, 1502, 1504, 1511, 1514, 1516, 1517, 1535, 1537, 1538, 1540, 1541, 1641, 1672, 1777, 1786, 1837, 1838, 1839, 1841, 1842, 1843, 1844, 1845, 1846, 1847, 1848, 1849, 1867, 1868, 1869, 1870, 1871, 1872, 1873, 1874, 1875, 1876, 1877, 1878, 1879, 1880, 1881, 1911, 1912, 1913, 1914, 1915, 1919, 1920, 1922, 2068, 2093, 2094, 2096, 2097, 2099, 2102, 2103, 2104, 2106, 2107, 2117, 2118, 2120, 2121, 2122, 2127, 2128, 2129, 2134, 2135, 2136, 2148, 2167, 2168, 2171, 2182, 2184, 2187, 2193, 2196, 2206)
                                     and TYPE != 'other' or 'raw count';") #longer site dataset is because of those from WP that have no entities.
str(RPDent)
...
write.csv(RPDdat, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Edat.csv", row.names = F)
write.csv(RPDsite, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Esite.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Eent.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Echron.csv", row.names = F)
...
ent <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Eent.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Edat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/TENA_Echron.csv' , na.strings = 'NULL')
...
dir.create("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/TENA_E")
csvpath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/TENA_E/"
...
for (j in maxsites[72:132]) { ##trying looping using entity names: changed from j in 1:maxsites
   nsamp <- 0
  sitedata <- data_query[data_query$ID_ENTITY == j, ]
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/TENA_E/",entid,"_RPD_", ".csv", sep="")
...
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/TENA_E/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
transcsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/ZTrans_TENA_E/",sep="") # 
transcsvpath
...
write.csv(ent_vec, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/TENA_E/final_entities.csv")
