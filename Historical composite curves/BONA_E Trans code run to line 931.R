...
RPDent <-  dbGetQuery(mydb, "SELECT * FROM entity WHERE ID_ENTITY in(271, 302, 324, 456, 457, 467, 704, 705, 1369, 1370, 1368, 1371, 1372, 1373, 1374, 1523, 1332, 1333, 1421, 1460, 1572, 1364, 1439, 1389, 1390, 1393, 1396, 1398, 1399, 1404, 1411, 1387, 1417, 1488, 1490, 1491, 1508, 1518, 1521, 1536, 1539, 1542, 1543, 1850, 1851, 1852, 1853, 1854, 1855, 1856, 1857, 1858, 1859, 1860, 1861, 1862, 1923, 2108, 2109, 2110, 2111, 2112, 2113, 2114, 2115, 2116, 2429, 2433);") #longer site dataset is because of those from WP that have no entities.
str(RPDent)
...
RPDsite <- dbGetQuery(mydb, "SELECT s.* FROM site s
                      ...
                      WHERE e.ID_ENTITY in(271, 302, 324, 456, 457, 467, 704, 705, 1369, 1370, 1368, 1371, 1372, 1373, 1374, 1523, 1332, 1333, 1421, 1460, 1572, 1364, 1439, 1389, 1390, 1393, 1396, 1398, 1399, 1404, 1411, 1387, 1417, 1488, 1490, 1491, 1508, 1518, 1521, 1536, 1539, 1542, 1543, 1850, 1851, 1852, 1853, 1854, 1855, 1856, 1857, 1858, 1859, 1860, 1861, 1862, 1923, 2108, 2109, 2110, 2111, 2112, 2113, 2114, 2115, 2116, 2429, 2433);") 
...
write.csv(RPDdat, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Edat.csv", row.names = F)
write.csv(RPDsite, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Esite.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Eent.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Echron.csv", row.names = F)
...
ent <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Eent.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Edat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/BONA_Echron.csv' , na.strings = 'NULL')
...
dir.create("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_E")
csvpath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_E/"
...
for (j in maxsites[]) { ##trying looping using entity names: changed from j in 1:maxsites
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_E/",entid,"_RPD_", ".csv", sep="")
...
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_E/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
transcsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/ZTrans_BONA_E/",sep="") # 
transcsvpath
...
write.csv(ent_vec, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/BONA_E/final_entities.csv")
