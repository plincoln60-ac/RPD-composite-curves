...
#CODE NOT USED AS WE'RE NOT USING BACON CHRONS
#need to isolate as a separate dataframe only these entities with the new model, then merge.----
d.tmp <- data_query[data_query$ID_SAMPLE %in% bacon_chrons$sample_ids,]
bac.tmp <- bacon_chrons[ bacon_chrons$sample_ids %in% data_query$ID_SAMPLE,]
length(unique(d.tmp$ID_SAMPLE))
length(unique(bac.tmp$sample_ids))
check  <- bac.tmp[!bac.tmp$sample_ids %in% d.tmp$ID_SAMPLE,] 
check <- bac.tmp[duplicated(bac.tmp$sample_ids),]
s <- bac.tmp[bac.tmp$sample_ids %in% check$sample_ids,]
s <- s[order(s$sample_ids),]


############################# Don't think we need these data as we're using the original ages
...
check <- data_query.m[is.na(data_query.m$TYPE),]
...
for (j in sites) { ##trying looping using entity names: changed from j in 1:maxsites
  nsamp <- 0
  sitedata <- data_query[data_query$entity_name == j, ]
...
if (nsamp > 1) { ##I changed this from 0 to 1, because we have entities that are only one row in v4.
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste(csvpath,entname,"_data.csv", sep="")
...
length(unique(data_query$ID_ENTITY))
datapath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/Transformed data/"
...
sitecsvpath
transcsvpath <- paste(datapath,"Ztrans_RPD_ent/",sep="") # 
transcsvpath

# if output folder does not exist, create it
#dir.create(file.path(datapath, paste(queryname,"_trans_csv/",sep="")), showWarnings=FALSE)
statscsvpath <- paste(datapath,"Stats_RPD_ent\\",sep="") #to store the stats
# if output folder does not exist, create it
...
sites <- sites[sites$ID_ENTITY %in% ls3,]
...
for (z in 1:nsites) {  #Do not change this counter to the name. very dangerous becuase the matrices you use are indexed by this counter as well.
  ...
  inputfile <- paste(sitecsvpath, entid,  "_data", date, ".csv", sep="")
...
for (j in 1:nsites) {  #looping through each .csv file of each site.
  ...
  inputfile <- paste(sitecsvpath, entid,  "_RPD_", date, ".csv", sep="")
  ...
  siteout <- data.frame(cbind(sitedata$ID_ENTITY, sitedata$ID_SAMPLE, sitedata$est_age, 
  ...
  outputfile <- paste(transcsvpath, entid, "_Ztrans_RPD_", basename, "_RPD_", date, ".csv", sep="") 
...
inputfile <- paste(transcsvpath, entid, "_Ztrans_RPD_", basename, "_RPD_",date, ".csv", sep="")
