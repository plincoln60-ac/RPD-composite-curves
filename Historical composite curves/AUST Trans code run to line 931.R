...
RPDent <-  dbGetQuery(mydb, "SELECT * FROM entity WHERE ID_ENTITY in(232, 237, 257, 258, 259, 262, 276, 277, 278, 282, 283, 284, 285, 289, 293, 304, 399, 400, 401, 402, 411, 430, 431, 432, 433, 436, 437, 438, 444, 470, 471, 472, 473, 474, 475, 496, 497, 498, 499, 501, 502, 503, 506, 508, 510, 513, 514, 516, 517, 591, 600, 602, 608, 610, 611, 612, 613, 614, 617, 618, 619, 620, 648, 650, 652, 654, 655, 656, 665, 668, 669, 670, 674, 675, 676, 677, 678, 682, 689, 707, 708, 709, 710, 720, 722, 724, 726, 687, 728, 732, 733, 734, 735, 797, 754, 756, 757, 758, 759, 760, 761, 762, 763, 764, 765, 767, 768, 770, 777, 832, 1001, 1740, 1741, 2151, 2217, 2235, 2298, 2302, 2308, 2312, 2313, 2318, 2319, 2340, 2342, 2343, 2348, 2354, 2355, 2356, 2358, 2365, 2367, 2370, 2375);") #longer site dataset is because of those from WP that have no entities.
str(RPDent)
...
RPDsite <- dbGetQuery(mydb, "SELECT s.* FROM site s
                      ...
                      WHERE e.ID_ENTITY in(232, 237, 257, 258, 259, 262, 276, 277, 278, 282, 283, 284, 285, 289, 293, 304, 399, 400, 401, 402, 411, 430, 431, 432, 433, 436, 437, 438, 444, 470, 471, 472, 473, 474, 475, 496, 497, 498, 499, 501, 502, 503, 506, 508, 510, 513, 514, 516, 517, 591, 600, 602, 608, 610, 611, 612, 613, 614, 617, 618, 619, 620, 648, 650, 652, 654, 655, 656, 665, 668, 669, 670, 674, 675, 676, 677, 678, 682, 689, 707, 708, 709, 710, 720, 722, 724, 726, 687, 728, 732, 733, 734, 735, 797, 754, 756, 757, 758, 759, 760, 761, 762, 763, 764, 765, 767, 768, 770, 777, 832, 1001, 1740, 1741, 2151, 2217, 2235, 2298, 2302, 2308, 2312, 2313, 2318, 2319, 2340, 2342, 2343, 2348, 2354, 2355, 2356, 2358, 2365, 2367, 2370, 2375);") 
...
write.csv(RPDdat, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTdat.csv", row.names = F)
write.csv(RPDsite, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTsite.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTent.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTchron.csv", row.names = F)
...
ent <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTent.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTdat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/AUSTchron.csv' , na.strings = 'NULL')
...
dir.create("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/AUST")
csvpath <- "/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/AUST/"
...
for (j in maxsites[123:126]) { ##trying looping using entity names: changed from j in 1:maxsites
  nsamp <- 0
  sitedata <- data_query[data_query$ID_ENTITY == j, ]
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
   ...
   #delete any rows in this data that have zero and negative sed rate, and save them so you can see how many are removed.
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/AUST/",entid,"_RPD_", ".csv", sep="")
...
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/Influx conversion/AUST/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
transcsvpath <- paste("/Users/paullincoln/Dropbox/2021/September 2021/Composite code v2/ZTrans_AUST/",sep="") # 
transcsvpath
