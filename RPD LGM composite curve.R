...
where e.ID_ENTITY in (select e.ID_ENTITY from entity e
left join sample s on e.ID_ENTITY = s.ID_ENTITY
left join age_model am on am.ID_SAMPLE = s.ID_SAMPLE
...
RPDsite <- RPDsite[RPDsite$ID_SITE %in% RPDent$ID_SITE,] ## sites.

#################

##now get the sample data for all these sites. should preserve sample IDs so you can eventually bind bacon chron.

RPDsample <-  dbGetQuery(mydb, "SELECT s.*, e.TYPE FROM sample s
                         left join entity e on e.ID_ENTITY = s.ID_ENTITY;")
RPDsample <- RPDsample[RPDsample$ID_ENTITY %in% RPDent$ID_ENTITY,]

length(unique(RPDsample$ID_ENTITY))

RPDchron <- dbGetQuery(mydb, "SELECT * FROM age_model;")
...
head(RPDdat)
RPDdat <- RPDdat[,c('ID_SITE', 'ID_ENTITY', "ID_SAMPLE", 'entity_name',"avg_depth",'charcoal_measurement','mean','TYPE')]

write.csv(RPDdat, "//Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMdat.csv", row.names = F)
write.csv(RPDent, "/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMent.csv", row.names = F)
write.csv(RPDchron, "/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMchron.csv", row.names = F)


##############################################################################################----


ent <- read.csv('/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMent.csv', na.strings = 'NULL')

dat <- read.csv('/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMdat.csv' , na.strings = 'NULL')

chron <- read.csv('/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMchron.csv' , na.strings = 'NULL')
...
str(data_query)
data_query$mean.x <- as.numeric(data_query$mean.x)
...
for (j in maxsites[]) { ##trying looping using entity names: changed from j in 1:maxsites
  ...
  if (nsamp > 1) { ##I changed this from 0 to 1, because we have entities that are only one row in v4.
    ...
    # local variables
    depth <- sitedata$avg_depth; age <- sitedata$mean.x; quant <- sitedata$charcoal_measurement; xst_level <- sitedata$TYPE.x; sampid <- sitedata$ID_SAMPLE
...
length(unent) # 109 entities remain, check which entities were lost

write.csv(unent, '/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMstage1entities.csv')
...
newdf<-newdf[newdf$est_age != -9999, ]                          # Removes all rows with intcal 13 calibration row based on condition

# query label and path to query and query name
dir.create("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Influx conversion/")

csvpath <- "/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Influx conversion"
...
head(data_query)
names(data_query) <- c("entity_name", "ID_ENTITY"  ,'ID_SAMPLE', "sample_depth"  ,     "mean"  ,   "quantity"  ,"TYPE" ) #sample ID
head(data_query)
...
str(data_query)
#INFLUX LOOP----
datapath<-"/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Influx conversion/"
for (j in maxsites[]) { ##trying looping using entity names: changed from j in 1:maxsites
  nsamp <- 0
  sitedata <- data_query[data_query$ID_ENTITY == j, ]
  ...
  if (nsamp > 1) { ##I changed this from 0 to 1, because we have entities that are only one row in v4.
    ...
    # local variables
    depth <- sitedata$sample_depth; age <- sitedata$mean; quant <- sitedata$quantity; xst_level <- sitedata$TYPE 
    ...
    {
      conc <- quant
      conc_source <- "count assumed as conc"
      if (conc != miss && sed_rate != miss && sed_rate != 0.0) {
        influx <- quant * sed_rate
        influx_source <- "calculated from conc "
      } else {
  ...
  if (nsamp > 1 && sum(influx) > 0) { ##Quality control bit after z score calculation: I changed nsamp from >0 to >1. 
    ...
    neg.infl <- append(neg.infl, outdata[outdata$sed_rate <= 0,])
    outdata <- outdata[!(outdata$sed_rate <= 0),]
    csvfile <- paste(datapath,entid,"_RPD_", ".csv", sep="")
...
print(ls3,)
write.csv(ls3, '/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGMstage2entities.csv')
length(ls3)
###############################################################################################################################
##########################################################################################################################################################
...
dir.create("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/0_4kv2")
sitecsvpath <- paste("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Influx conversion/",sep="") #where all the csvs that have the influx data are stored.
sitecsvpath
dir.create("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/0_4kv2/ZTrans_LGM_O_4kv2")
transcsvpath <- paste("/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/0_4kv2/ZTrans_LGM_O_4kv2",sep="") # 
transcsvpath
...
write.csv(ent_vec, "/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/LGM_final_transformed_entities.csv")

##########################################  END OF MAIN LOOPING BIT ###################################################################



########################################## GROUP ENTITIES INTO SINGLE FILE ###################################################################
...
dir.create('/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Regional_transformed_data')

write.csv(LGM_dataset, './LGM_datasetv2.csv')
...
setwd("/Users/paullincoln/Dropbox/2021/Research/RPD LGM for model comparison/ZTrans_LGMv2/")

dat <- read.csv('LGM_datasetv2.csv')
dir.create("/Users/paullincoln/Dropbox/2021/Research/RPD LGM for model comparison/Composite_curve")
...
Zbin_dat <- Zbinning(dt = dat, bins = seq(250, 25000,500), binhw = 500) #for now will go with  bin width of 250
colnames(Zbin_dat)

#give ages of each bin as a column

bin_age <- seq(0,25000,500)
...
#subset dataframe to just include 'LGM' values

dat_LGM <- dat %>% filter(EST_AGE > 17000)
dat_LGM <- dat_LGM %>% filter(EST_AGE < 24000)
dat_LGM 
LGM_comb <-left_join(comb, dat_LGM, by = "ID_SAMPLE")


ggplot(LGM_comb, aes(x = mean.BA, y= zt, color = mean)) +geom_point()
###################################################################################################
...
dir.create( "/Users/paullincoln/Dropbox/2021/Research/RPD LGM for model comparison/Composite_curve/")
datapath <- "/Users/paullincoln/Dropbox/2021/Research/RPD LGM for model comparison/Composite_curve/"
...
targbeg <- 0
targend <- 25000
targstep <- 500 ## this is based on the previous binning procedure
...
if (plotout == "pdf") { ## when you define PDFfile over here, need to make sure you name it with the nsites and what baseperiod overlap level you insisted on.
  pdffile <- paste(curvecsvpath,"LGM_",basename,"_",binname,"_",as.character(hw),"_",as.character(nreps),'_',as.character(length(Zbin_dat)-1),'n','20_1_2022',".pdf", sep="")
  print(pdffile)
}
...
plot(x, y, xlab="Age (BP 1950)", ylab=fitname, xlim=c(25000, 0), xaxp  = c(25000,0, 25), ylim=c(-2,1), type="n") ##make sure xlim is right.
title(main = 'LGM_scores, bp = 0-25000BP: min 50% overlap, bins = 27, hw = 500, nboot = 1000', cex.main = 0.6)
...
ggplot(dat, aes(EST_AGE, influxmnx, color = ID_ENTITY))site_LGM$mean <- site_LGM$mean /1000
ggplot(dat ,aes(x=EST_AGE, y= influxmnx)) + geom_line() + facet_wrap(~ID_ENTITY, scales= 'free_y') + scale_x_continuous(limits = c(0,25000))

NGRIP<-read.csv('/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/NGRIP_data.csv')
Osman <- read.csv('/Users/paullincoln/Dropbox/2022/Research/LGM paper & new code/Osman et al.csv')
Osman2 <-ggplot(Osman, aes(x=agemid, y= X50th)) + theme_bw() + geom_ribbon(aes(ymin = X5th, ymax= X95th, alpha = 0.2, fill = 'blue'))+ geom_ribbon(aes(ymin = X30th, ymax= X70th, alpha = 0.5, fill = 'blue'))  + geom_line() + scale_x_continuous(limits = c(0,25000), breaks = seq(0,25000,1000))
NGRIP2 <-ggplot(NGRIP, aes(x = t, y=d18O)) + geom_errorbarh(aes(xmin = t - Years_dt, xmax= t + Years_dt, alpha = 0.5, colour = 'blue')) + geom_line() + theme_bw() + scale_x_continuous(limits = c(0,25000), breaks = seq(0,25000,1000))
composite2 <-ggplot(curveout, aes(x = age, y=locfit)) +geom_ribbon(aes(min = cl95, ymax= cu95, fill = 'red', alpha = 0.5)) + geom_line() + theme_bw()+ scale_x_continuous(limits = c(0,25000), breaks = seq(0,25000,1000))

library(tidyverse)
library(ggpubr)
ggarrange(composite2, Osman2, NGRIP2, align = 'v', 
               labels = c("a)", "b)", "c)"),
               ncol = 1, nrow = 3)
